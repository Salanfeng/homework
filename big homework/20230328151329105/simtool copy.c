...
int Findhash()
{
        ...
        else
        {
            if (flag != 1)
            {
                ...
                if (FindStop(word) == 1)
                {
                    ...
                    /*for (j = 0; j < HashMAX && (hashword[j].count > 0); j++)
                    {
    ...
    qsort(hashword, HashMAX, sizeof(struct HashWord), CompareWordCount);
    fclose(article);
    FILE *hashfile;
...
void Deal(struct Word *ArtWord)
{
    ...
    qsort(ArtWord, ArtMAX, sizeof(struct Word), CompareWordCount);
    for (int i = 0; i < N && ArtWord[i].count > 0; i++)
    {
        for (int j = 0; j < N; j++)
        {
...
void Dealsample(char *name, struct Word *SamWord)
{
    ...
    qsort(SamWord, ArtMAX, sizeof(struct Word), CompareWordCount);
    for (int i = 0; i < N && SamWord[i].count > 0; i++)
    {
        for (int j = 0; j < N; j++)
        {
